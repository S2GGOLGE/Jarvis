...
from config.app_config import has_gemini_api_key, load_app_config, save_app_config
...
class SoundManager:
    ...
    def _start_sound_process(self, path: Path, volume: float):
        ps_script = (
            "Add-Type -AssemblyName PresentationCore; "
            f"$p = New-Object System.Windows.Media.MediaPlayer; "
            f"$p.Open([Uri]'{path.as_uri()}'); "
            f"$p.Volume = {max(0.0, min(1.0, volume)):.2f}; "
            "$p.Play(); "
            "while ($p.NaturalDuration.HasTimeSpan -eq $false) { Start-Sleep -Milliseconds 50 }; "
            "$d = [Math]::Max(1, [int]$p.NaturalDuration.TimeSpan.TotalMilliseconds); "
    ...
    def _loop_ambient(self, stop_event: threading.Event):
        while not stop_event.is_set():
            with self._lock:
                ...
                volume = self._volume
            try:
                proc = self._start_sound_process(_HUD_FILE, volume)
            except Exception:
                break
    ...
    def _foreground_worker(
        ...
        while not stop_event.is_set():
            try:
                proc = self._start_sound_process(path, volume)
            except Exception:
                break
...
class JarvisUI:
    def __init__(self):
        ...
        # Windows pencere yoneticisi bazen geometry'yi override eder, tekrar zorla.
