...
Not:
- Otomatik gönderim ve kişi arama için PyAutoGUI kullanilir.
"""
...
from actions.windows_utils import click_active_window, copy_to_clipboard, hotkey, open_app, open_url, press_enter


PREFERRED_BROWSERS = ["default browser"]
AUTO_SEND_DELAY_SECONDS = 4.0
...
def _auto_send_with_keyboard(app_name: str = "") -> tuple[bool, str]:
    return press_enter(delay=AUTO_SEND_DELAY_SECONDS)
...
def _open_whatsapp_chat_by_name(recipient_name: str) -> tuple[bool, str]:
    ok, detail = open_app("WhatsApp")
    if not ok:
        return False, f"WhatsApp Desktop acilamadi: {detail}"

    _copy_to_clipboard(recipient_name.strip())
    hotkey("ctrl", "n", delay=2.0)
    hotkey("ctrl", "a", delay=0.4)
    hotkey("ctrl", "v", delay=0.1)
    press_enter(delay=1.2)
    return True, f"WhatsApp Desktop uzerinden {recipient_name.strip()} sohbeti acildi."


def _open_whatsapp_desktop_by_name(recipient_name: str, message: str, send_now: bool) -> tuple[bool, str]:
    ok, detail = _open_whatsapp_chat_by_name(recipient_name)
    if not ok:
        return False, detail
    ...
    hotkey("ctrl", "v", delay=1.2)
    if send_now:
        press_enter(delay=0.3)
...
def send_whatsapp_message(
    ...
    if app_target in {"auto", "desktop"}:
        if normalized_phone:
            ...
            if ok:
                ...
                ok_send, send_detail = _auto_send_with_keyboard("WhatsApp")
                if ok_send:
                    label = resolved_name or f"+{normalized_phone}"
                ...
                return (
                    ...
                    f"{send_detail}. WhatsApp penceresi odakta olmayabilir veya PyAutoGUI gerekli olabilir."
                )
        ...
        if resolved_name:
            ...
            if app_target == "desktop":
                return (
                    ...
                    f"{detail}. WhatsApp icindeki kisi adi eslesmesi gerekli olabilir."
                )
    ...
    ok_send, send_detail = _auto_send_with_keyboard(detail)
    if ok_send:
        label = resolved_name or f"+{normalized_phone}"
        source_note = " (rehberden bulundu)" if contact_source == "phone_book" else ""
    ...
    return (
        ...
        f"{send_detail}. Tarayici penceresi odakta olmayabilir veya PyAutoGUI gerekli olabilir."
    )
...
def call_whatsapp_contact(
    ...
    call_type = (call_type or "voice").strip().lower()
    ...
    if normalized_phone:
        ok, detail = _open_whatsapp_desktop_via_scheme(normalized_phone, "")
        label = resolved_name or f"+{normalized_phone}"
    elif resolved_name:
        ok, detail = _open_whatsapp_chat_by_name(resolved_name)
        label = resolved_name
    else:
    ...
    # WhatsApp Desktop'ta arama tuslari pencerenin sag ustunde bulunur.
    # Resmi bir call URL scheme olmadigi icin bu en iyi-caba arayuz otomasyonudur.
    x_from_right = 104 if call_type == "voice" else 152
    ok_click, click_detail = click_active_window(x_from_right=x_from_right, y_from_top=54, delay=2.0)
    if ok_click:
        kind = "sesli" if call_type == "voice" else "goruntulu"
        return f"WhatsApp uzerinden {label} icin {kind} arama baslatilmaya calisildi."
    return (
        f"WhatsApp sohbeti acildi ama arama butonuna basamadim: {click_detail}. "
