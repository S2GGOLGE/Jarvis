...
from actions.whatsapp  import send_whatsapp_message, save_whatsapp_contact, call_whatsapp_contact
...
OWNER_UNLOCK_RE = re.compile(r"^\s*(?:jarvis\s+)?(?:yetki|owner|sahip)\s+(?:kodu|pin|şifre|sifre)\s+(.+?)\s*$", re.IGNORECASE)
OWNER_LOCK_RE = re.compile(r"^\s*(?:jarvis\s+)?(?:kilitle|oturumu\s+kapat|owner\s+lock)\s*$", re.IGNORECASE)
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                "command": {
                    "type": "STRING",
                    "description": "Calistirilacak PowerShell veya cmd komutu"
                }
            },
            "required": ["command"]
        }
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                "pin": {
                    "type": "STRING",
                    "description": "Sahip PIN/yetki kodu"
                }
            },
            "required": ["pin"]
        }
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                "action": {
                    "type": "STRING",
                    "description": "stop | pause | resume | play_pause | next | previous | mute | volume_up | volume_down"
                }
            },
            "required": ["action"]
        }
